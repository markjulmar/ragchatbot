...
class RAGSystem:
    ...
    def _execute_sequential_rounds(self, query: str, history: Optional[str]) -> Tuple[str, List[str]]:
        """
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            messages.append({"role": "assistant", "content": self.ai_generator.last_tool_response})
            messages.append({"role": "user", "content": self.ai_generator.last_tool_results})
    ...
    def _detect_tool_loop(self, current_calls: List[Dict], previous_calls: List[Dict]) -> bool:
        """
        ...
        for current_call in current_calls:
            for previous_call in previous_calls:
                if (current_call.get('name') == previous_call.get('name') and 
                    current_call.get('input') == previous_call.get('input')):
                    return True
        return False
